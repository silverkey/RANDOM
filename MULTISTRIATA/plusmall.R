...
write.table(res,file='plusmall_top15.xls',sep="\t",row.names=F,quote=F)

pdf(bg="transparent",width=8,height=10,pointsize=1,paper='a4',file='plusmall_top15.pdf')
par(mfrow=c(4,2))
for(i in 1:nrow(res)) {
  ID=res[i,1]
  ann = anno[anno$Identifier==ID,]
  main = c(substr(ann[,'Uniref.Desc'],1,80),substr(ann[,'CDD.Desc'],1,80))
  dotchart(as.numeric(eset[ID,]),labels=colnames(eset),color=c(2,1,1,1,1,2),main=main,cex.main=0.9,cex=1.05)
}
dev.off()
