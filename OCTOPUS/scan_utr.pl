...
# DEFAULTS
my $DIR = '/media/LOCAL_DATA/RESEARCH/ANALYSIS/OCTOPUS/ASSEMBLY_FROM_TANTO/miRNA/';
my $UTR_FASTA = 'octomodels_filtered.fa';
my $MIR_FASTA = 'human_mirnas_fasta';
my $OUT = 'octopus_miRNA_scan.xls';
my $MIN = 1;
...
while(my $seq = $UTR->next_seq) {
  foreach my $id(keys %$mirna_seqs) {
    ...
    my $tot = ($counts->{'7mer-A1'}->{count}+$counts->{'7mer-m8'}->{count}+$counts->{'8mer'}->{count});
    my @res = ($seq->id,$id,$counts->{'7mer-A1'}->{count},$counts->{'7mer-m8'}->{count},$counts->{'8mer'}->{count},$tot);
    my $newrow = join("\t",@res);
    print OUT "$newrow\n" if $tot >= 1;
  }
}
