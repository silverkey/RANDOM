...
open(OUT,">$outfile");
my @OUTheader = qw(id chr start end strand length tr.id tr.name tr.chr tr.start tr.end tr.strand);
...
while(my $element = <IN>) {
  ...
  foreach my $or(@$overlap) {
    ...
    my @ovrow = ($id,$chr,$start0,$end,$strand,($end-$start0),$or->{name},$or->{name2},$or->{chrom},$or->{txStart},$or->{txEnd},$or->{strand});
...
sub get_overlap {
  ...
  my $sth = $DBH->prepare("SELECT * FROM refGene WHERE chrom = \'$chr\' AND ".
                          "txStart <= $end AND txEnd >= $start0");
  $sth->execute;
  ...
  while(my $row = $sth->fetchrow_hashref) {
    ...
    my $r2 = Bio::Range->new(-start => $row->{txStart},
                             -end => $row->{txEnd});
    ...
    # Here we can put some kind of analysis about the transcript
