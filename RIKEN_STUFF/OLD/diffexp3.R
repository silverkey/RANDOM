...
out.tab  = 'repeat_expdiff_results_PAR.txt'
...
d = DGEList(eset,group=target$chem_time)
d = calcNormFactors(d)
d = estimateCommonDisp(d)
res = exactTest(d,pair=c('TSA_2h','VEHtsa_2h'))
...
boxplot.region = function(id,colnames) {
...
barplot.region = function(id,colnames) {
  data = matrix(as.numeric(raw[raw$id==id,][-1]),ncol=length(colnames))
  colnames(data) = colnames
  barplot(data,col=8,cex.axis=0.6,main=paste(id,'normalized counts'),beside=T)
}
