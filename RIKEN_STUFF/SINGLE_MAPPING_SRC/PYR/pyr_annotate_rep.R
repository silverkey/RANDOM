...
coord = read.delim(file='pyr_coord_filt.txt')
sel = read.delim(file='pyr_expdiff_results.xls')
out.file = 'rep_pyr_expdiff_results.xls'

get.overlap = function(chr,start,end) {
  query = paste(sep='',"SELECT DISTINCT repName,repClass,repFamily ",
                       "FROM ",chr,"_rmsk WHERE ",
                       "genoStart <= ",end," AND ",
                       "genoEnd >= ",start)
...
colnames(res) = c('id','repName','repClass','repFamily')
