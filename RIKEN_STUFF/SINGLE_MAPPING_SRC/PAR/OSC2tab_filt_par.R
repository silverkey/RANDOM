...
osc.name = 'pv_all_6_level2.osc'
exp.field = 'ncig100'
norm.field = 'norm'
raw.field = 'raw'
del.1 = 'norm.ncig10000_'
del.2 = '.notrim.+sorted'
del.3 = 'raw.ncig10000_'
...
# consistent with the previous OSC
colnames(norm.tab) = gsub('vEH_','veh_',colnames(norm.tab),perl=T)
...
colnames(raw.tab) = gsub('vEH_','veh_',colnames(raw.tab),perl=T)
...
write.table(coord.tab,file='par_coord.txt',sep="\t",quote=F,row.names=F)
write.table(norm.tab,file='par_norm.txt',sep="\t",quote=F,row.names=F)
write.table(raw.tab,file='par_raw.txt',sep="\t",quote=F,row.names=F)
write.table(target,file='par_target.txt',sep="\t",quote=F,row.names=F)

# Filter based on total reads per row
rsum = rowSums(raw.tab[,-1])
write.table(coord.tab[rsum>=min.reads,],file='par_coord_filt.txt',sep="\t",quote=F,row.names=F)
write.table(norm.tab[rsum>=min.reads,],file='par_norm_filt.txt',sep="\t",quote=F,row.names=F)
write.table(raw.tab[rsum>=min.reads,],file='par_raw_filt.txt',sep="\t",quote=F,row.names=F)
